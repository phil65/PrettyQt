...
class ProxyMapper(core.Object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._proxy_1 = proxy_1
        self._proxy_2 = proxy_2
        proxychain_1 = [proxy_1] + proxy_1.get_source_models()
        proxychain_2 = [proxy_2] + proxy_2.get_source_models()
        common_list = [c for c in proxychain_1 if c in proxychain_2]
        if not common_list:
            raise RuntimeError("No common source model")
        self._common_source = common_list[0]
        logger.debug(f"Common source: {self._common_source}")
        self._proxychain_1 = proxychain_1[: proxychain_1.index(self._common_source)]
        self._proxychain_2 = proxychain_2[: proxychain_2.index(self._common_source)]
