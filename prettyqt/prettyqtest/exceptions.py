...
def capture_exceptions():
    ...
    Returns them as a list of (type, value, traceback) after the context ends.
    """
...
class _QtExceptionCaptureManager:
    ...
    def start(self):
        ...
        Record exceptions received into ``self.exceptions``.
        """
    ...
    def fail_if_exceptions_occurred(self, when):
        """Call pytest.fail() with a message if exceptions have been captured so far.

        Before pytest.fail() is called, also finish capturing.
        """
        if self.exceptions:
            self.finish()
...
def format_captured_exceptions(exceptions) -> str:
    """Format exceptions given as (type, value, traceback) into a string.
...
def _is_exception_capture_enabled(item):
    """Return if exception capture is disabled for the given test item."""
    disabled = item.get_closest_marker("qt_no_exception_capture") or item.config.getini(
